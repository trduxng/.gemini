...
- **Surgical Reading:** Tuyệt đối không đọc toàn bộ file lớn. Luôn chỉ đọc đoạn code cần thiết.
- **Search-Driven Context:** Ưu tiên tìm kiếm có mục tiêu với số lượng kết quả thấp để định vị mục tiêu trước.
- **Progressive Expansion:** Tìm trong phạm vi hẹp trước; chỉ mở rộng khi không đủ dữ liệu.
- **Compressed Delegation:** Chỉ dùng sub-agent cho tác vụ thật sự nặng hoặc cần cô lập ngữ cảnh; kết quả trả về phải là bản tóm tắt súc tích nhất.
- **Context Pruning:** Bỏ qua các file rác, build artifacts, và thư mục không liên quan bằng `.geminiignore` chặt chẽ.
- **Minimal Output:** Phản hồi văn bản ngắn gọn, ưu tiên kết quả, diff, test status, và bước tiếp theo.
- **Lowest context when possible:** Với tác vụ đơn giản, ưu tiên chế độ/context thấp nhất đủ để hoàn thành.
- **Do not over-read:** Nếu một đoạn 20-80 dòng đã đủ ngữ cảnh thì không đọc thêm nhiều file liên quan.
