...
- **Script Tự động Nạp (Auto-Loading):** Phải đánh giá `<RequiredSkills>` trong bước Thinking. Nếu thiếu công cụ chuyên dụng, BẮT BUỘC phải gọi lệnh `activate_skill <tên_nhóm>` NHƯ LÀ BƯỚC ĐẦU TIÊN trong Action Plan. Nếu không tìm thấy skill trong thư mục mặc định, AI phải thực hiện **On-demand Discovery**:
    1. Tham chiếu `skill_second.md` để xác định domain.
    2. Dùng `grep_search` hoặc `list_directory` để tìm đường dẫn chính xác trong `skills_second/`.
    3. Kích hoạt bằng đường dẫn đầy đủ.
